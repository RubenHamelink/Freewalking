...
namespace Freewalking.Loaders
{
    public class TerrainLoader : MonoBehaviour, ITerrainExtension
    {
        ...
        private readonly Dictionary<float, Dictionary<float, GameObject>> cells =
            new Dictionary<float, Dictionary<float, GameObject>>();
        ...
        public void CreateCellAt(Vector3 position)
        {
            ...
            if (!cells.ContainsKey(position.x))
                cells.Add(position.x, new Dictionary<float, GameObject>());

            cells[position.x].Add(position.z,
                GeneratePlane(new Vector3(position.x * CellSize, 0, position.z * CellSize), CellSize + 1));
        }

        public bool CellExistsAt(Vector3 position)
        {
            return cells.ContainsKey(position.x) && cells[position.x].ContainsKey(position.z);
        }
